  ...
  const handleFileUpload = (event) => {
    ...
    return (
        <div>
            ...
            {imagePreview.length > 0 && (
                <div className='mt-3 border-solid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-10 gap-3'>
                    {imagePreview.map((image, index) => (
                        <div key={index}>
                            <Image src={image} width={100} height={100} className="rounded-lg" alt={`preview-${index}`} />
                        </div>
                    ))}
                </div>
            )}
            ...
            {imageList && imageList.length > 0 && (<div className='mt-3 border-solid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-10 gap-3'>
                {imageList.map((image, index) => (  // ✅ imageList, not imagePreview
                    <div key={index}>
                        <Image src={image?.url} width={100} height={100} className="rounded-lg" alt={`saved-${index}`} />
                    </div>
                ))}
            </div>
            )}
